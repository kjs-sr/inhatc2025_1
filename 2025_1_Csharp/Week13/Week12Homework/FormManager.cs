    ...
    public partial class FormManager : Form
    {
                ...
                try
                {
                    using (var fs = new FileStream(filename, FileMode.Open))
                    {
                        using (var sr = new StreamReader(fs))
                        {
                            ...
                            {
                                var data = sr.ReadLine();
                                var stud = Student.Restore(data, departments);
                                if (stud != null)
                                {
                                    students[stud.Number] = stud;
                                    lbxDictionary.Items.Add(stud);
                                }
                            }
                        }
                    }
        ...
        private void btnRemoveDepartment_Click(object sender, EventArgs e)
        {
            ...
            if (lbxDepartment.SelectedItem is Department) {
                ...
                {
                    if (stud.Value.Dept.Code == target.Code)
                    {
                        MessageBox.Show("사용중인 학과 코드입니다.");
                        return;
                    }
        ...
        private void btnRemoveProfessor_Click(object sender, EventArgs e)
        {
            ...
            if (lbxProfessor.SelectedItem is Professor) {
                ...
                foreach (var stud in students)
                {
                    if (stud.Value.AdvisorNumber == target.Number)
                    {
        ...
        */
        ...
        private void UpdateInfo(Dictionary<string, Student> students, string filename)
        {
            ...
            {//열고
                using (var sw = new StreamWriter(fs))
                {
                    ...
                    {
                        if (stud.Value != null)
                            sw.WriteLine(stud.Value.Record);
                    }
                }
            }
        }
        ...
        private void tpgDepartment_Click(object sender, EventArgs e)
        {
